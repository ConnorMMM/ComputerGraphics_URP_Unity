        ...
        for(int x = 0; x < _density; x++)
        {
            for(int y = 0; y < _density; y++)
            {
                ...
                for(int i = 0; i < _settings.octaves; i++)
                {
                    ...
                    float simplex = Remap(noise.srnoise(new float2(sampleX, sampleY)), -1, 1, 0, 1);
                    
                    float perlin = Remap(noise.cnoise(new float2(sampleX, sampleY)), -1, 1, 0, 1);

                    float t = _settings.interpolationCurve.Evaluate(i / _settings.octaves);
                    
                    noiseHeight += (Mathf.Lerp(simplex, perlin, t)) * amplitude;
